...
import { FlaskConical, Play, AlertTriangle, CheckCircle, Database, RotateCcw } from 'lucide-react';
    ...
    try {
      ...
      if (response.data && response.data.resultado) {
        ...
        toast.success('Simulación de laboratorio completada', {
          description: `Actividad máx: ${metricas.actividad_maxima || 'N/A'}%, Eficiencia: ${metricas.eficiencia_cataltica || 'N/A'}%`
        });
      }
    } catch (error: any) {
    ...
    return (
      <div className="p-8 space-y-6">
        <div>
          ...
          <p className="text-gray-600">Simulación de experimentos in silico con modelos cinéticos (HU-008)</p>
        </div>
        <Card>
          <CardHeader>
    ...
    <div className="p-8 space-y-6">
      ...
      {sequences.length > 0 && selectedSequenceIndex < sequences.length && (
        <ReportDownloader 
          ...
          hasPlm={false}
          hasLaboratory={currentSimulation !== null}
